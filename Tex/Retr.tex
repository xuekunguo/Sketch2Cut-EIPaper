...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{候选部件搜索}
本章中，我们介绍如何从未做预分割的模型数据库内快速、精确地搜索出与代理模型相匹配的候选部件。
...
广义柱体的两端可以放缩，得到圆锥或圆台。
创建薄板模型，用户首先在当前模型上画一条曲线，再画出薄板模型轮廓线。
该轮廓线位于用户视线方向与曲线起点定义的空间平面上。
我们的系统允许用户对三种基础代理模型施加放缩、切割、变形等操作。更复杂的代理模型可以通过基础代理组合得到。
%--------------------------
...
体积变化次之的视角下提取的轮廓是副轮廓，用于候选部件排序。最后，我们采用第\ref{SubSecCtourDesc}节提出的方法提取主、副轮廓的描述符（采样距离为9）。

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{候选部件搜索}\label{SubSecPartRetr}
给一个三维代理模型与一个数据库模型，我们的目标是在数据库模型上找到与代理模型最相似的部分（称为候选部件）。
...
\caption{三维模型超面片图建立过程示意图。我们将三维模型（a）分割成200个片断（c）。
一个片断视为一个超面片，做为超面片图上一个顶点。在超面片图上，相邻超面片间用带权重的边相连。}\label{FigSFG}
\end{figure}
...
其中$G = \left\{ {{g_i}|1 \le i \le N} \right\}$是一次分割结果的所有片断。为了得到超面片图上边的权重，我们首先将相关的两超面片在不同分割结果上的概率累加，再通过分割次数归一化。
%--------------------------
\begin{figure}\centering
\includegraphics[width=\linewidth]{./Mat/SFDist.pdf}
\caption{超面片在分割片断上的分布示意图。$s_0$，$s_1$和$s_2$是超面片。$g_0$，$g_1$与$g_2$是通过Randomized Segmentation~\cite{Golovinskiy2008}得到的分割片断。\TODO{这个图左侧[s0,s2]改样子，右侧改数据。}}\label{FigSFDist}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{候选部件搜索}\label{SubSubSecPartRetr}
给一个候选轮廓及数据库模型，我们的目标是在数据库模型上找到与候选轮廓最匹配的候选部件。
...
\caption{候选部件搜索算法流程。给一个候选轮廓（（a）内红色部分），我们首先计算外超面片obj-prob（b）；
然后计算内超面片obj-prob（c）；最后通过graph-cut技术得到整个候选部件（d）。
\TODO{优化该图；颜色条字再大些。inner super-face, outer super-face}}\label{FigPartRetr}
\end{figure}
...
对外部超面片（自身轮廓与模型轮廓有重叠的超面片），obj-prob通过其自身轮廓与候选轮廓的重合程度得到。
超面片图内，除了外部超面片之外的超面片称为内部超面片。我们认为，在超面片图内外部超面片与内部超面片的obj-prob相一致。
因此，整张图内的obj-prob可以形化为：
\[{\bf{AX}} = {\bf{b}},\]
条件为：
\[{\bf{X}} \ge 0,\]
...
%\includegraphics[width=1.0\linewidth]{./Mat/PartRetr.pdf}
\missingfigure{}
\caption{\TODO{类似sketch-to-design Fig 4，来张图，展示三个量}}\label{FigPartRetr}
\end{figure}
我们基于候选部件与代理部件的兼容性对候选部件排序。在计算兼容性时，我们考虑如下因素：1）候选部件的二维投影与代理部件的相似性；
2）候选部件与代理部件的几何相似性；3）候选部件的背景与代理部件的相似性。
给一个候选部件$p$（$p$取自数据库模型$M$）与代理部件$B$，两者的兼容性定义如下：
\[J\left( {p,B} \right) = \omega {T_p}\left( {p,B} \right) + \xi {T_g}\left( {p,B} \right) + \psi {T_c}\left( {p,B} \right),\]
其中$\omega=0.8$，$\xi = 0.15$，$\psi = 0.05$。$T_p \left( \cdot, \cdot \right)$计算模型二维投影的相似性。
$T_g \left( \cdot, \cdot \right)$计算模型的几何相似性。$T_c \left(, \right)$计算模型背景的相似性。
...
候选部件基于兼容性分数由高低排列。如果用户需要多样性的部件建议，我们采用技术Maximal Marginal Relevance criterion~\cite{Chaudhuri2010}排列候选部件。