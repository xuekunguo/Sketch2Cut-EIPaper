...
其中$J_{\theta_i}^j$表示在采样距离$\theta_i$下最好的第$j$个匹配结果（见公式（\ref{EquCtourMatch}））。
...
然后计算内超面片obj-prob（c）；最后通过graph-cut技术得到整个候选部件（d）。}\label{FigPartRetr}
\end{figure}

我们采用min-cut/max-flow技术~\cite{Boykov2004}求解方法。n-links的容量是边权重。t-links的容量定义为：
...
我们认为，外部超面片与内部超面片（除了外部超面片之外的）的obj-prob相一致。
因此，整个超面片图的obj-prob可以形化为：
\[{\bf{AX}} = {\bf{b}},\]
条件为：
\[{\bf{X}} \ge 0,\]
其中${\bf{A}} = {\bf{I}} - {\bf{A}}'$，$\bf A'$是带权重的内部超面片间的相邻矩阵。$\bf X$是内部超面片obj-prob值。
...
%\includegraphics[width=1.0\linewidth]{./Mat/PartRetr.pdf}
\missingfigure{}
\caption{\TODO{类似sketch-to-design Fig 4，来张图，展示三个量}}\label{FigPartRank}
\end{figure}
我们基于候选部件与代理部件的兼容性对候选部件排序。在计算兼容性时，我们考虑如下因素：
1）候选部件的二维投影与代理部件的相似性；
...
候选部件基于兼容性分数由高低排列。
如果用户需要多样性的部件建议，候选部件采用Maximal Marginal Relevance criterion技术~\cite{Chaudhuri2010}排列。 