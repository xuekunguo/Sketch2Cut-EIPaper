...
\title{Sketch2Cut：针对浏览式草图造型的按需部件建议技术}
\author{Xuekun Guo}
\date{2016.04.25}
...
\section{相关工作}
%--------------------------
\paragraph{探索式建模技术} 在过去几年内，将创造力支持因素与传统的模型概念设计相结合是一个非常活跃的话题。
...
Kalogerakis et al.~\cite{aprobabilisticmodelvladlensg2012}，Averkiou et al.~\cite{AverkiouShapeSynth2014}及其它研究人员提出了用于模型组合的概念推理模型。

我们的方法属于部件提示技术范畴。然而，我们工作的主要技术贡献是我们不依赖预分割（或标记）的三维模型数据库，即时地从数据库内提取出与用户草图相匹配的部件。
因此，我们的方法允许用户高效地访问更大的建模空间。
我们的工作与Modeling by Example~\cite{modelingbyexamplefunkhousersg2004}不同，我们通过二维草图创建三维代理，自动提取可用部件，同时也需要数据库模型做一致性对齐。

%--------------------------
\paragraph{模型语义分割技术} 将模型按语义分割是图形学领域的基础问题。研究人员提出了各种各样不同的技术，包括自动化的或手动的~\cite{surveyonmeshsegmentationshamircgf2008,benchmarkforsegmentationchenxiaobaisg2009}。
最近，研究人员又提出针对一组模型做共分割（co-segmentation）的技术~\cite{learning3dmeshsegmentationkalosg2010,cosegmentationof3dshapesliuligangcgf2012,jointshapesegmentationhuangqixingsg2011,SidKaiKleZhaCoh11}。
共分割技术将一组模型视为一个整体，分割的过程中，可以利用该组模型的共有结构，因此得到更加一致性的结果。
这些工作可用于自动化地预分割模型数据库，利于基于部件组合的探索式建模技术的发展。

与以上这些技术不同，我们的方法并不需要预分割与标记的三维模型数据库。我们基于用户输入的草图即时分割模型。因此，我们的分割过程必须足够快，达到实时。
更进一步，提取出的部件必须与用户草图相匹配，而用户输入的草图我们没法预计出来。这就要求我们提出一种即时的轮廓感知的模型分割算法。

%--------------------------
\paragraph{基于草图的模型搜索技术} 随着三维模型数据越来越多，快速且方便的基于内容的模型检索技术~\cite{asurveyofcontentbasedtangeldermultimedia2008}越来越重要。
基于内容的检索技术通常需要用户提供一个三维模型做为检索关键字。在三维建模中，这一要求形成循环依赖。
相对的，基于草图的检索技术~\cite{asearchenginefunkhousertog2003,discriminativesketchbasedshaotianjiacgf2011,EitRicBouHilAle12}允许用户画出期望模型在一个或多个视角下的大致轮廓。
在表达用户意图方面，该方法相对更加直观与方便。

Lee et al.~\cite{sketchbasedcompositionfunkhousersbim2008}将基于草图的模型检索与基于组件的三维建模融合到一个框架中。
Xie et al.~\cite{sketchtodesignxukaicgf2013}提出一个通过上下文感知的部件替换技术做模型编辑的系统。
...
然后，我们打算在当前框架内加入部件融合功能~\cite{meshcompositionjunconglintvcg2008,meshmixersinghsgtalk2010}。
